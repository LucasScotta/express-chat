...
module.exports = class Room {
  ...
  setProperties(ID,suscriptors,displayMessage, feedOptions) {
    ...
    if (!id || typeof id !== 'number') return Error(`Expected number, but got ${typeof number}`)
    if (!suscriptors || typeof suscriptors !== 'object') return Error(`Expected object, but got ${typeof suscriptors}`)
    if (!displayMessage || typeof displayMessage !== 'string') return Error(`Expected string, but got ${typeof displayMessage}`)
    if (!feedOptions || typeof feedOptions !== 'object') return Error(`Expected object, but got ${typeof feedOptions}`)
    this.id = id
