  ...
  getByPass: function(id, pass, callback) {
    users
      ...
      .then(result => {
        ...
        if(data.usuario === id && data.password === pass) {
          return callback(null, {name: data.usuario, pass: data.password, blockeds: data.blockeds, silenced: data.silenced, friends: data.friends, friendRequests: data.friendrequest, createdAt: data.created_at, updatedAt: data.updated_at})
        }
        if(data.usuario === id && data.password !== pass) {
          return callback(new Error('Contrasena incorrecta'))
          // return callback(new Error('Usuario o contrasena incorrectos'))
        }
      })
      .catch(() => {
        return callback(new Error('Usuario invalido'))
      })
  },
